...
When we prefer a step that is intentionally meant to serve a specific task, we not 
only provide better readability, but we also increase the chance that the underlying
...
Without taking some precautions, such a social network, modeled as a graph, could
face issues.
...
looking for. If we did not include the calls to 'next' we would actually get back the
...
missing vertex, but other structure as well, in this case another vertex and an edge
between them.
...
such as JanusGraph, will require you to do make some decisions in this area. Those 
...
labels. You can collect an label distribution using 'groupCount' for vertices or
edges.
...
As well as generating maps, we can also generate a set using a 'aggregate' step so
...
exactly what is aggregated. The query below uses a 'aggregate' step to create a
...
we'd made a mistake and mistyped '"runways"' as '"rnways"' we would have no results:
...
This behavior is meant as a convenience when using 'aggregate' with heterogenous or
...
achieve what you need. It is fair to observe that sometimes coming up with a closure
to do what you want is easier than figuring out the pure Gremlin way of doing it.
However, using just Gremlin steps is still the recommended path to take. Lambda
