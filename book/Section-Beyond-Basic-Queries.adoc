...
a traversal does not make it a <<var,terminal step>>, i.e. it does not iterate the
traversal. Since 'drop' returns no results, the usual terminating step to use in
conjunction with it is 'iterate'.

Despite the 'drop' filter behavior, we'd still like to be able to remove a property
and then add a new one in the same traversal. In the "<<sideeffect,Using 'sideEffect'
to do things on the side>>" section, we introduced how 'sideEffect' can be used to
include additional processing without changing what gets passed on to the next stage
of the query. We can wrap 'drop' in this step to handle the property removal as a
side-effect of the traversal execution.
