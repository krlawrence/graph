...
configured, the file 'remote.yaml' can be used. we had to make one change, as shown
below. We commented out the 'serializer' line and replaced it with a slightly
modified version. We had to do this because of the version issues we mentioned above.
At time of writing, JanusGraph supports version V1d0 of the GyroMessageSerializer
that is used as the communication serialization protocol between the Gremlin Console
and the Gremlin Server. However, our Gremlin Console was pre-configured with the
newer version V3d0 of the serializer. It is essential that the console and the server
be using the same version. If we did not make this change, the Gremlin Console and
...
returned as part of the V1 format. We find that users find this format much easier to
