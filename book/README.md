...
## Agent-friendly Markdown (llms.txt)
...
- `target/md/` — a split Markdown mirror with one page per chapter (large chapters
  are split further so each page stays under an agent-sized budget).

These are generated by `bin/make-llms.sh`, which uses
[`downdoc`](https://gitlab.com/antora/downdoc) (an npm package) for the
AsciiDoc-to-Markdown conversion and validates the result with
[`afdocs`](https://agentdocsspec.com/) (`npx afdocs check`). Both require
[Node.js](https://nodejs.org/); everything is written to the git-ignored `target/`
directory and is regenerated by the CI workflow on each push to `main`.

```shell
# Produce target/md/ and target/llms.txt, then validate against the spec
./bin/make-llms.sh
```

Curated one-line page descriptions come from `// llms-summary:` comments placed above
section headings in the `Section-*.adoc` files; sections without one fall back to
