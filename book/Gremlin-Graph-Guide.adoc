...
v278-preview, Mar 17, 2018
// Sat Mar 17, 2018 13:43:20 CDT
...
Sometimes it is useful to be able to remember (label) a point of a traversal and
refer back to it later on. This ability was more essential in TinkerPop 2 than it is
in TinkerPop 3 but it still has many uses. The query below uses an 'as' step to
attach a label at two different parts of the traversal, each representing different
vertices that were found. A 'select'
step is later used to refer back to them. 
...
'by' modulator can be used to specify which property to retrieve from the selected
vertices.
...
work. For completeness, here is the same query but using 'path'. You will see the
both the 'select' and 'path' steps used a lot throughout this book as appropriate.
...
the same label. In the example below, the label '+++'+++a+++'+++' is used twice but
you will notice that when the label is selected only the last item added is returned.
...
Keywords such as these are documented in the "<<javastatics>>" section later on in
the book.
