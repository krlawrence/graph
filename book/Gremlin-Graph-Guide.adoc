...
The next query looks for routes that follow the pattern A->B->C but where there is
...
the airport of origin. Notice how a 'where' step is included to make sure we do not
match any routes of the form A->B->A.
