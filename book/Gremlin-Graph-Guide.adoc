...
could take a lot longer.  The addition of a 'limit' step is also important as without
it this query will run for a LONG time looking for every possible path!! 

The query blow looks for routes between Austin (AUS) and Agra (AGR). An important
...
A bit later, in the <<emit>> section we will look at how 'emit' can be used to adjust the
behavior of a 'repeat...times' loop.
...
you run this query on your laptop, after a few minutes of hight CPU usage and
