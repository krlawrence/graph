...
v278-preview, Feb 14, 2018
// Wed Feb 14, 2018 09:32:25 CST
...
graph contains billions of vertices and edges. That said, I have some queries that
can take several minutes to run in that environment.
...
ground up with distributed systems, it is possible to use a stand a alone TinkerGraph
...
would be written if we were to use the Graph Computer more directly. The example
below sets up a 'PageRankVertexProgram' and runs it. Notice that the result we get
back includes a new graph with 3624 vertices and no edges.
...
In case you are curious, we can check the precise type of the result object that is returned
from creating our vertex program.
...
graph. As you can see there are no new properties in this version.
