...
v279-preview, Apr 27, 2018
//v278 (TP 3.3.1), Mar 28, 2018
// Fri Apr 27, 2018 09:45:31 CDT
...
:toc:
:toclevels: 4
:revision: 279-preview
//:revision: 278 (TP 3.3.1)
:doctype: book
:icons: font
//:pdf-page-size: Letter
:draftdate: April 27th 2018
...
looks for strings between '"Dal"' and '"Dam"' in an inclusive/exclusive fashion. The rest
of the characters following '"Dal"' in the strings being tested are ignored. Note
that this is a case sensitive comparison. In other words '"Dal"' and '"dal"' are
different strings in this context. 

TIP: The 'between' predicate can be used to simulate a string 'startsWidth' method.
...
simulate a 'startsWidth' type of operator.
...
You will notice from the results above that '"Dallas"' appears twice as there are tow
...
If the graph database you are using supports user provided ID values, you can use an 'inject'
step as one way to specify what you want the ID value of a new vertex to be. For
...
Note that if you try to create a vertex using an ID that already exists the operation
will fail.
...
As an interesting sidenote, and this would be true for the queries above as well, if
...
Using the prior query the first result was '[DXB,8818,AKL]'. As you will
see below, our first result is now '[AKL,8818,DXB]'.   
