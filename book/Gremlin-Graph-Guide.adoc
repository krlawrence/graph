...
v277-preview, Feb 8, 2018
// Thu Feb 08, 2018 18:53:40 CST
...
All that we have to do to establish a connection between our Gremlin Console and the
Gremlin Server is to issue the folowing command once the console is running.
...
Given that the air-routes graph s already loaded into our remote graph we can
immidiately start to issue some queries. In order to make sure the query goes to the
...
conventinet to put the console into '"remote mode"'. This can be done as follows.
...
One thing to notice is that the output that comes back froma Gremlin Server looks
a little different from when you use the commands using the Gremlin Console attached
to a TinkerGraph. This is because Gremlin Server essentially does a 'toString()' on
the output before it is returned to the console.
...
Once you are done sending all commands to the Gremlin Server you can disable that
mode as follows.
...
the JSON generated also contains a map.
