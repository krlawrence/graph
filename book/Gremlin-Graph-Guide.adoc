...
v277-preview, Jan 18, 2018
// Thu Jan 18, 2018 18:57:46 CST
...
We can also generate a set using a 'store' step so that duplicate values are not
stored. The 'withSideEffect' step can be used to initialize the set. The 'cap' step
emits the collection resulting from the side effect that we created using the 'store'
step. Among other things this allows us to return this collection as the final result
of a query.
...
If we are ever unsure what type of object has been created can a call to
...
To access the keywords 'keys' and 'values' can be used to access the respective
...
recall from our earlier discussion that vertex properties are stored as arrays even
if there is only one property - an array of length one in other words. Note that this
...
We could add a second 'unfold' to just get the city names back and remove the arrays.
...
If we were not sure how many times we need to 'unfold' we can change the query as
follows. The 'repeat' loop will unfold until there is only a list of arrays left. The
final unfold will remove the remaining arrays leaving us with just the text values.
...
because while most of property keys are Strings, IDs and labels are a special
case.
