...
If all we wanted back was a single list of key value pairs, another way we could do
it is to use a 'map' and not use a 'sack' step at all as shown below. There are other
ways you might chose to write a query like this, that avoid the use of closures, but
this again provides an interesting example.
