...
the book releases in various formats (HTML,PDF, DocBook/XML, MOBI and EPUB) at the
same GitHub location. The sample programs include stand alone Java and Groovy
examples as well as many examples that can be run from the Gremlin Console.The sample
applications area contains a full example HTML and JavaScript application that lets
you explore the air-routes graph visually. Below are some links to each.
