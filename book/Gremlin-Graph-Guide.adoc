...
v280-preview, June 1, 2018
//v279 (TP 3.3.3), May 31, 2018
// Fri Jun 01, 2018 14:59:42 CDT
...
When run, the query returns all airports with codes that start win the letter '"X"'.
...
You can use the 'cyclicPath' step to find paths through the graph that end up back
where they started. The rather contrived example below finds routes that both start
and end in Austin (AUS).
...
Note that this has the same effect as if we had written the query as shown below.
