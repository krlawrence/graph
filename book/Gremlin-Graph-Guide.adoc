...
location.  The sample programs include standalone Java, Groovy and Ruby examples as
well as many examples that can be run from the Gremlin Console.  There are some
differences between using Gremlin from a standalone program and from the Gremlin
...
much look at different ways to work with graph data stored in text format
...
NOTE: There are additional 'WithOptions' values we can use to change how the way
...
Continuing with our example from the previous section, using the new 'with' step and
specifying options using 'WithOptions' we can generate a result from 'valueMap' that
includes the values and their meta properties. The example below will include the
values for all properties in the map plus the key and value for any meta properties.
...
|incr    | Order.asc      | order().by(Order.asc)
