...
v277-preview, Feb 2, 2018
// Fri Feb 02, 2018 08:52:42 CST
...
[source,groovy]
----
gremlin> g.withSack(0).V().has('code','SAF').out().
         values('runways').fold()
                                                                                             `
[7,4,3,6]
----
...
part of the TinkerPop Java Enum called 'Operator'. Other possible operators that we
can use include 'div','min' and 'max'. The example below looks at the distances of
...
challenge is to write a query that shows 10 rootes that start at SAF and have one
stop. We also want to return the distance between each hop and the total distance of
the two hops. This is a perfect example of a query where using a 'sack' can help.
