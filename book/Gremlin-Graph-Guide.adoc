...
The title of this book could equally well be '"A getting started guide for users
of graph databases and the Gremlin query language featuring hints, tips and sample
...
practices, lessons I learned the hard way etc.
...
This book introduces the Apache TinkerPop 3 'Gremlin' graph query
...
The graph, 'air-routes.graphml', is a model of the world airline route network
...
console running with a TinkerGraph. 

NOTE: Currently the examples in this book have been tested using Apache Tinkerpop
version 3.2.5.  Some breaking changes were recently introduced in Apache Tinkerpop
3.3. The text will be revised ASAP to reflect those changes.
...
- In Chapter four we move beyond just reading the graph and describe how to add nodes,
...
information to the nodes and edges as needed. The 'directed' part of the name means
that any edge has a direction. It goes 'out' from one node and 'in' to another. You
will sometimes hear people use the word 'digraph' as shorthand for 'directed graph'.
Consider the relationship "Kelvin knows Jack". This could be modeled as a node for
each of the people and an edge for the relationship as follows. 
...
more information about them. For example, my age might be a property on my node. 
...
this document being a perfect example.

Those of you that looked at graphs as part of a computer science course we be correct
...
Server. We cover all of these topics in detail in this document.

We have tested the queries and other examples in this document against Apache
TinkerPop version 3.2.4 using the TinkerGraph graph and the Gremlin console as well
as some other TinkerPop 3 enabled graph stores.
...
As TinkerGraph is intended mostly for learning and testing it does not support some
...
is that 'UserSuppliedIds' is set to true for vertex and edge ID values. This means
...
we shall see later this is not the case with most other graph systems.

When running in the Gremlin Console, support for TinkerGraph is off by default. You
...
The air-routes graph contains several node types that are specified using labels. The
most common ones being 'airport' and 'country'. There are also nodes for each of the
seven continents ('continent') and a single 'version' node that I provided as a way
...
Each airport node has many properties associated with it giving various details about
that airport including its IATA and ICAO codes, its description, the city it is in
and its geographic location.

Specifically, each airport node has a unique ID, a label of 'airport' and contains
the following properties. The word in parenthesis indicates the type of the property.

----
 type    (string) : The node's type. Will be 'airport' for airport nodes   
...
Even though the airport node label is 'airport' I chose to also have a property
...
The graph currently contains a total of 3,612 nodes and 49,894 edges. Of these 3,367
...
----
Air Routes Graph (v0.76, 2017-Aug-09) contains:
  3,367 airports
  43,160 routes
...
Additional observations:
  ...
  Average route distance is 1,163.525 miles.
  ...
  Maximum airport node degree (routes in and out) is 541 (AMS)
  Country with the most airports: United States (579)
  Continent with the most airports: North America (977)
  Average degree (airport nodes) is 25.637
  Average degree (all nodes) is 25.770
----

Here are the Top 15 airports sorted by overall number of routes (in and out). In
graph terminology this is often called the degree of the node or just 'node degree'.

----
 POS  ID   CODE  TOTAL     DETAILS    
                                      
  1	  70   AMS  (541)  out:269 in:272
  2	 161   IST  (540)  out:270 in:270
  3	  52   FRA  (532)  out:266 in:266
  4	  51   CDG  (524)  out:262 in:262
  5	  80   MUC  (474)  out:237 in:237
  6	  64   PEK  (467)  out:233 in:234
 ...
 12	  67   PVG  (400)  out:200 in:200
 13	  50   LGW  (394)  out:197 in:197
...
disabled. Depending upon your use case you may or may not need to manage plugins.  In
TinkerPop 3 the 'sugar' plugin us disabled by default.You can enable 'sugar' support
from the Gremlin console as follows:
...
TinkerPop 2 provided by default a plugin called 'Tinkerpop.sugar' that allowed
shorthand forms of queries to be entered when using the Gremlin console. In TinkerPop
3 that plug-in is off by default and has to be enabled if you want to continue to use
the same shortcuts that TinkerPop 2 allowed by default. 
...
of loading a 'graph' we also setup a traversal called 'g' which we will then refer to
in our subsequent queries of the graph.  The 'max-iteration' option tells the Gremlin
console the maximum number of lines of output that we ever want to see in return from
a  query. The default, if this is not specified, is 100.

If you are using a different graph environment such as IBM-Graph you can still load
the 'air-routes.graphml' file by following the instructions specific to that system.
Once loaded, the queries below should still work either unchanged or with minor
modifications.
...
When using a TinkerGraph, the message you get back will look something like this.
...
document. This provides a nice way of getting back any routes between a source and
destination no matter how many hops it might take to get there.

Again, don't worry if some of the Gremlin shown here is confusing, we will cover it
in detail a bit later. The key point to take away from this discussion of SQL and
Gremlin is that for data that is very connected, Graph databases provide a very good
way to store that data and Gremlin provides a nice and fairly intuitive way to
traverse that data efficiently.

One other point worthy of note is that every node and every edge in a graph has a
...
to create a graph traversal object for our loaded 'graph'.
...
Once we have a graph traversal object we can use it to start exploring the graph.
...
set of nodes or edges we are interested in. That usage is explained in the "<<wid>>"
section.

TIP: If it helps with remembering you can think of 'g.V()' as meaning "looking at all
of the nodes in the graph" and 'g.E()' as meaning "looking at all of the edges in the
...
The query below will return any nodes (vertices) that have the 'airport' label.
...
Here is what we gt back from the query. Notice that this is the Gremlin Console's way
...
[[values]]
Retrieving property values from a node
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

There are several different ways of working with node properties. We can add, delete
...
of simple ways that we can look up the property values of a given node.

[source,groovy]
----
// What property values are stored in the DFW node?
...
[[exist]]
Does a specific property exist on a given node or edge?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
all lets find out how many nodes in the graph represent airports.
...
You could shorten the above a little as follows but this would caue more edges to get
looked as as we do not first filter out all nodes that are not airports.
...
[source,groovy]
----
// How many of each type of node are there?
...
[source,groovy]
----
[continent:7,country:237,version:1,airport:3367]
----
...
[EU:578,AS:931,NA:977,OC:284,AF:294,AN:0,SA:303]
----
...
So far we have mostly just explored queries that look at properties on a node or count
...
connections (edges) between nodes. The term 'walking the graph' is used to describe
moving from one node to another node via an edge. Typically when using the phrase
'walking a graph' the intent is to describe starting at a node traversing one or more
...
The 'out' step is used to find vertices connected by an outgoing edge to that node
and the 'outE' 'step' is used to explore outgoing edges from a given node. Conversely
the 'in' and 'inE' steps can be used to look for incoming nodes and edges. The 'outE'
...
between nodes and edges. We will encounter those in the "<<otherv>>" section of this
document. 

This first query does a few interesting things. Firstly we find the node representing
the Austin airport (the airport with a property of 'code' containing the value 'AUS').
Having found that node we then go 'out' from there. This will find all of the nodes
...
[[pathintro]]
What nodes and edges did I visit? - Introducing 'path'
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
of the nodes that are on the other ends of those edges".

[source,groovy]
----
// This time, for each route, return both nodes and the edge that connects them.
...
[[edgeexist]]
Does an edge exist between two nodes?
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
You can use the 'hasNext' step to check if an edge exists between two nodes and 
...
Retrieving a range of nodes
^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Gremlin provides various ways to return a sequence of nodes. We have already seen the
...
range of nodes by giving a non zero starting offset and an ending offset. The 'range'
...
[source,groovy]
----
// Return the first two airport nodes found 
g.V().hasLabel('airport').range(0,2)

// Return the fourth, fifth and sixth airport nodes found (zero based)
g.V().hasLabel('airport').range(3,6)

// Return all the remaining nodes starting at the 3500th one
g.V().range(3500,-1)
----

NOTE: There is no guarantee as to which airport nodes will be selected as
...
achieve the same results that you can get from the combination of 'as' and
'select' steps. The example below shows the previous query, rewritten to use 'project'
instead of 'as' and 'select'.
...
[[vm]]
Using 'valueMap' to explore the properties of a node or element
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

A call to 'valueMap' will return all of the properties of a node or element as an
...
the map unless a parameter of 'true' is provided.

[source,groovy]
----
// Return all the properties and values the AUS node has
...
The last example selects three properties to be in the value map.
...
Every node and every edge in a graph has a unique ID that can be used to reference
...
finds the ID, which is 8, for the node that represents the DFW airport.

[source,groovy]
----
 // What is the ID of the "DFW" node?
 g.V().has('code','DFW').id()

 8
----

Let's reverse the query and find the code for the node with an ID of 8.
...
[source,groovy]
----
// Nodes with an ID between 1 and 5 (note this is inclusive/exclusive)
g.V().hasId(between(1,6))

// Which is an alternate form of this
g.V().has(id,between(1,6))

// Find routes from the node with an ID of 6 to any node with an ID less than 46
...
[source,groovy]
----
// What is the code property for the node with an ID of 3?
g.V(3).values('code')

AUS

// As above but for all of the specified node IDs
...
It's a good idea when designing a graph to give the nodes and edges meaningful
labels. You can use these to help refine searches. For example in the air-routes
graph, every airport node is labelled 'airport' and every country node, not
...
By using labels in this way we can effectively group nodes and edges into classes or
types.  Imagine if we wanted to build a graph containing different types of vehicles.
We might decide to label all the nodes just 'vehicle' but we could decide to use
...
nodes that have the 'airport' label for a code of "'SYD'".
...
[source,groovy]
----
// Select some nodes at random and return them with their elevation.
g.V().hasLabel('airport').coin(0.05).values('code','elev').fold()
----

We can see how fairly the 'coin' step is working by counting the number of nodes
...
If all you want is, say 20 randomly selected nodes, without worrying about
...
Given the air routes graph has over 3,300 airport nodes I wanted to come up with a
...
lot easier. The query below starts at the Austin airport and finds all the airports
that you can fly to from there. A 'where' 'by' step is then used to filter the
results to just those airports that have the same number of runways that Austin has.
What is really nice about this is that we do not have to know ahead of time how many
runways Austin itself has as that is handled for us by the query.
...
The next example looks for routes that follow the pattern A->B->C but where there is
no direct flight of the form A->C. In other words it looks for all routes between two
...
g.V().hasLabel('airport').match(__.as('a').out().as('b')
                               ,__.as('b').out().is(neq('a')).as('c')
                               ,__.not(__.as('a').out().as('c')))
                         .select('a','b','c').by('code')
----
...
[source,groovy]
----
g.V().has('code','AUS').match(__.as('aus').values('runways').as('ausr'),
...
Hopefully, however, these examples give you an idea for what 'match' can do.
...
Here is what we get back when we run or query
...
After the first 'out' step. all of the nodes that were found are stored in a
collection I chose to  call 'nonstop'. Then, after the second 'out' we can add a
'where' step that essentially says "only keep the nodes that are not part of the
...
[[otherv]]
Other ways to explore nodes and edges using 'both', 'bothE', 'bothV' and 'otherV'
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

We have already looked at examples of how you can walk a graph and examine nodes and
edges using steps such as 'out', 'in', 'outE' and 'inE'. In this section we
introduce some additional ways to explore nodes and edges.
...
We can use a repeat...until looping construct to look for paths between two airports 
without having to specify an explicit number of 'out' steps to try.

The use of 'simplePath' prevents paths already taken from being taken again and
greatly speeds up this query.  Without the 'simplePath' step being used this query
could take HOURS!!  The 'limit' step is also important as without it this query will
run for a LONG time looking for every possible path!! 

[source,groovy]
----
// What are some of the shortest routes (paths) from AUS to AGR?
...
g.V().has('code','AUS').until(has('code','SYD')).repeat(out().simplePath()).limit(10).path().by('code')    

// We can also specify an explicit number of out steps to try using a repeat...times loop but this of course
// assumes that we know ahead of time how many stops we want to look for between airports.
g.V().has('code','AUS').repeat(out()).times(2).has('code','SYD').path().by('code')

// This is the same as the prior query (but less flexible in that we can less easily vary the number of out steps()):
...
g.V().has('code','AUS').in()
----
That query does not cause an error and correctly returns all of the nodes that are
...
Take a look at the query below. We first look just for nodes that are continents. We
...
In the first example it is likely that a lot less nodes will get looked at
...
referred to as 'supernodes'. In some cases the presence of 'supernodes' may be
unavoidable but with careful planning as you design your graph model you can reduce
the likelihood that nodes become 'supernodes'. The reason we worry about 'supernodes'
...
look at most if not all of the edges connected to that node as part of a traversal.
...
----
This example returns a hash of nodes, with  node labels as the keys and the 
...
[source,groovy]
----
// Create a map (a) of all nodes with the code property as the key
...
[[addnodes]]
Adding nodes, edges and properties 
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Nodes and edges can be added directly to the graph using the 'graph' object or as
part of a graph traversal. We will look at both of these techniques over course of
the following pages.

Adding an airport (node) and a route (edge)
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
from it to the existing DFW node. We can specify the label name ('airport') and as
...
In many cases it is more convenient to perform each of the previous operations using
just the traversal object 'g'. The following example does just that. We first create
a new aiport node for our imaginary airport and store it's vertex in the variable
'xyz'. We can then use that stored value when we create the edge also using a
...
[[addloop]]
Adding nodes and edges using a loop
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
Sometimes it is more efficient to define the details of the nodes or edges that
...
[source,groovy]
----
nodes = [["WYZ","KWYZ"],["XYZ","KXYZ"]]
for (a in nodes) {graph.addVertex(label,"airport","code",a[0],"iata",a[1])}
----

We could also have added the nodes using the traversal object 'g' as follows. Notice
the call to 'next()'.  Without this the node creation will not work as expected.

[source,groovy]
----
nodes = [["WYZ","KWYZ"],["XYZ","KXYZ"]]
for (a in nodes) {g.addV("airport").property("code",a[0],"iata",a[1]).next()}
----

This technique of creating nodes and/or edges using a 'for' loop can also be useful
...
----
nodes = [["WYZ","KWYZ"],["XYZ","KXYZ"]]
nodes.each {g.addV("airport").property("code",it[0],"iata",it[1]).next()}
----

[[deleting]]
Deleting nodes, edges and properties
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
Removing all the edges or nodes in the graph
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
You could also delete the whole graph, nodes and edges, by deleting all of the
nodes!
...
In a 'property graph' both nodes and edges can contain one or more properties. We
...
We have seen many examples already that show how both nodes and edges have a unique
...
tinkergraph[vertices:3612 edges:49748]
----
...
flowing in to it. The 'it' is Groovy syntax for the thing that came in (in this case
a vertex. The 'get' is needed to gain access to the properties of the vertex inside
of a lambda expression. Lastly we get the 'id' of the vertex and add one to it. The
modified values are then passed on to the next step of the traversal where they are
made into a list by the 'fold' step.
...
International Airport' in the case of DFW. If we wanted to search the nodes in
...
// Use it to find any nodes where the description string starts with 'Dal'
...
connected by routes to the nodes in the previous level. 
...
You can create a sub graph which is a subset of the nodes and edges in the main
...
[source,groovy]
----
// Create a subgraph of all nodes and edges directly connected to the AUS node
...
Because the air-routes graph also has country specific nodes, we could chose to write
the previous query a different way. We could start by finding country nodes and then
see how many airport nodes each one is connected to.
...
'groupCount' step is then used to group the nodes we visited by a count of the number 
of runways each has. We then go 'out' again and count how many nodes we found and
...
property of the respective nodes and edges.
...
Here are some more queries that are based on the distance between airports.

[source,groovy]
----
// Routes Between 100 and 200 miles in length 
g.V().as('src').outE().has('dist',within(100..200)).as('e').inV().as('dest')
     .select('src','e','dest').by('code').by('dist')

// Routes Between 100 and 200 miles in length, but only within the US.
g.V().has('airport','country','US').as('src').outE().has('dist',within(100..200)).as('e')\
     .inV().has('country','US').as('dest')\
     .select('src','e','dest').by('code').by('dist')

// Return a list of routes and their distances, starting from San Antonio (SAT)
g.V().has('code','SAT').as('src').outE('route').as('e').inV().as('dest')
     .select('src','e','dest').by('code').by('dist')
----

Calculating the distance between two directly connected airports is very easy. All we
have to do is look at the 'dist' property of the edge that connects them. We could do
this using the 'select' technique or in more recent versions of TinkerPop we could
use 'path' and 'by'.
...
g.V().has('airport','code','AUS').outE().inV().outE().inV().has('code','LHR').path().by('code').by('dist')
----
...
g.withSack(0).V().has('code','AUS').outE().sack(sum,'dist').inV().
                                    outE().sack(sum,'dist').inV().has('code','LHR').
                                    sack().order().by(incr).limit(10).
...
[source,groovy]
----
g.withSack(0).V().has('code','AUS').outE().sack(sum,'dist').inV().outE().sack(sum,'dist').inV().has('code','LHR').sack().order().by(incr).limit(10).path().by('code').by('dist').by('code').by('dist').by('code').by().toList().each(){println "${it[0]} --> ${it[2]} --> ${it[4]} ${it[5]} miles"}
----
...
uses the 'region' property that is present on all of the airport nodes in the graph.
...
Here is the output we get from running the query. As you can see, the nodes and edges
...
graph as there are clearly many ways to revisit nodes.
...
node and then randomly goes to five connected nodes from there.  The 'random walk' is
...
Changes to 'sack' syntax in TP 3.3
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Starting with Tinkerpop version 3.3 a change was introduced to the way that 'sack' is
used.

[source,groovy]
----
g.withSack(0).V().has('code','AUS').outE().sack(sum).by('dist').inV().outE().sack(sum).by('dist').inV().has('code','LHR').sack().order().by(incr).limit(10).path().by('code').by('dist').by('code').by('dist').by('code').by()
...
Warning about 'path' being memory intensive
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^

Running a query with point of funnel first works fine but put AUS first and will
run out of memory.

[source,groovy]
----
g.V().has('code','PSV').repeat(out().simplePath()).until(has('code','AUS')).limit(10).path().by('code')
...
[source,groovy]
----
// Using next to iterate through nodes
...
In reality, for a commercial application you would not use TinkerGraph as your graph
...
a stand alone Java application that uses Gremlin and TinkerGraph.
...
Using the management API you can define the labels that will be used by nodes and
...
The examples shown above of how a CSV file can be used to store data about nodes and
...
each row represent all of the other nodes that this node is connected to.
