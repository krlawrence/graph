...
v278-preview, Mar 18, 2018
// Sun Mar 18, 2018 10:29:09 CDT
...
Obviously there are a lot of ways that 'from' and 'to' could be used. By way of one
...
identical you may want to remove duplicates. That is where the 'dedup' step is
...
----
g.V().has('airport','code','AUS').out().as('a').out().as('b').out().
      path().by('code').from('a').to('b').limit(10).dedup()
...
throughout the remainder of this book. However, here are a few things to be aware of
