...
requires a recent version of Java 8 being installed. We have done all of our testing
...
In the current revision of this book we have tried to remove any dependence on the
...
form. In the current revision we have tried to remove all use of that form. It's
possible that we may have missed a few and we will continue to check for, and fix,
any that got missed. Please let me know if you find any that slipped through the
net!_
...
TinkerGraph does have some indexing capability we have also included some discussion
of it in the "<<tinkerindex>>" section. In Chapter 6 where we start to look at
additional technologies such as JanusGraph we have included a more in depth
discussion of indexing as part of that coverage. You should always refer to the
specific documentation for the graph system you are using to decide what you need to
do about creating an index and schema for your graph. we will explain what
TinkerGraph is in the next section. We won't be discussing the creation of an
explicit schema again until Chapter 6. When working with TinkerGraph there is no need
to define a schema ahead of time. The types of each property are derived at
creation time. This is a really convenient feature and allows us to get productive
and do some experimenting really quickly.
...
index should be considered a best practice. As we mentioned, even TinkerGraph has a
