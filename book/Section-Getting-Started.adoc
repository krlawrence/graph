...
using TinkerGraph The nice thing about TinkerGraph is that for learning and testing
things you can run everything you need on your laptop or desktop computer and be up 
and running very quickly. We will also explain how to get started with the Gremlin 
Console and TinkerGraph a bit later in this section.
...
To keep things consistent, all the examples presented in this book were produced
using the 1.0 version of the air routes data set generated October 2025. The 1.0
version is likely the last version of this data set to be produced. The form it is
in for this version is rich enough for demonstration purposes and serves as an
excellent resource for learning Gremlin and graphs. Having a stable 1.0 version means
that any educational resources adopting this version in different contexts will
simply just work when taken to other ones.
...
The following code loads the air routes graph using the Gremlin Console by putting it
into a file and using ':load' to load and run it or by entering each line into the
console manually. These commands will setup the console environment, create a
TinkerGraph graph and load the `air-routes.graphml` file into it. Some extra console
