...
take a look at the sample code we have included with the book.
...
located at https://github.com/krlawrence/graph/tree/main/sample-code that
...
typically be returned is shown below. We included a version check in the output so in
case this changes in the future you can see which version of Gremlin we queried.
...
In the Java example, we included the CLASSPATH on the 'javac' invokation using the
